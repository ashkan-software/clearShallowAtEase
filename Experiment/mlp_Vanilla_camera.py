...
def define_vanilla_model_MLP(input_shape, num_classes, hidden_units=32):
...
def define_MLP_architecture_edge(edge_input, hidden_units, output_layer_name):
    edge_output = Dense(units=hidden_units, name=output_layer_name, activation="relu")(
        edge_input
    )
    return edge_output


def define_MLP_architecture_fog_with_two_layers(
    fog_input, hidden_units, output_layer_name, input_layer_name
):
    fog = Dense(units=hidden_units, name=input_layer_name, activation="relu")(fog_input)
    fog_output = Dense(units=hidden_units, name=output_layer_name, activation="relu")(
        fog
    )
    return fog_output
...
def define_MLP_architecture_fog_with_one_layer(
    fog_input, hidden_units, output_layer_name
):
    fog_output = Dense(units=hidden_units, name=output_layer_name, activation="relu")(
        fog_input
    )
...
def define_MLP_architecture_cloud(cloud_input, hidden_units, num_classes):
    cloud = Dense(units=hidden_units, name="cloud_input_layer", activation="relu")(
        cloud_input
    )
