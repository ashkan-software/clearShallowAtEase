...
from Experiment.ResNet_blocks import (
    _bn_relu,
    _bn_relu_conv,
    _conv_bn_relu,
...
def define_vanilla_CNN_ResNet(
    ...
    # IoT Node
    iot = define_cnn_architecture_IoT(
...
def define_cnn_architecture_IoT(
    img_input, initial_filters, initial_kernel_size, initial_strides
):
...
def define_cnn_architecture_edge(
    iot_output,
    r,
    transition_dilation_rate,
    block_fn,
    ...
    if PARTITION_SETING == 1:
        ...
        edge, filters = _helper_define_conv_blocks(
            edge,
            0,
            2,
            r,
...
def define_cnn_architecture_fog(
    edge_output, r, transition_dilation_rate, block_fn, filters, dropout, residual_unit
):
    if PARTITION_SETING == 1:
        fog_output, filters = _helper_define_conv_blocks(
            edge_output,
            1,
            7,
            r,
...
def define_cnn_architecture_cloud(
    ...
    if PARTITION_SETING == 1:
        cloud, filters = _helper_define_conv_blocks(
            fog_output,
            2,
            12,
            r1,
    ...
    if include_top and top is "classification":
        ...
        cloud = layers.Dense(
            units=classes, activation=activation, kernel_initializer="he_normal"
        )(cloud)
    elif include_top and top is "segmentation":
        cloud = layers.Conv2D(classes, (1, 1), activation="linear", padding="same")(
            cloud
        )
...
def _helper_define_conv_blocks(
    input,
    i,
    block_id,
    r,
