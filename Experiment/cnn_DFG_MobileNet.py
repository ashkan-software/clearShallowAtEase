...
def define_cnn_DFG_architecture_IoT(input_shape, alpha, img_input, strides=(2, 2)):
    ...
    if strides == (1, 1):
        ...
        skip_iotfog = layers.Conv2D(
            96, (1, 1), strides=1, use_bias=False, name="skip_hyperconnection_iotfog"
        )(iot_output)
    elif strides == (2, 2):
        skip_iotfog = layers.Conv2D(
            96, (1, 1), strides=2, use_bias=False, name="skip_hyperconnection_iotfog"
        )(iot_output)
    else:
...
def define_cnn_DFG_architecture_edge(
    ...
    edge_output = define_cnn_architecture_edge(
        iot_output, alpha, depth_multiplier, strides=strides
    )
    ...
    skip_edgecloud = layers.Conv2D(
        384, (1, 1), strides=4, use_bias=False, name="skip_hyperconnection_edgecloud"
    )(edge_output)
