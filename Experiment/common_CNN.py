...
def define_model(iteration, model_name, dataset_name, input_shape, classes, alpha, strides, num_gpus, weights):
    ...
    if model_name == "DFG":
        if dataset_name == "cifar_resnet":
            ...
            model, parallel_model = define_DFG_CNN_MobileNet(classes=classes,input_shape = input_shape,alpha = alpha, strides = strides, num_gpus=num_gpus, weights=weights)
        model_file =  "models/"+ dataset_name  + str(iteration) + 'average_accuracy_DFG.h5'
    # Vanilla model
