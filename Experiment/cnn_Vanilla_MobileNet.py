...
def define_vanilla_CNN_MobileNet(
    input_shape=None,
    alpha=1.0,
    depth_multiplier=1,
    include_top=True,
    ...
    **kwargs
    ...
    cloud = define_cnn_architecture_cloud(
        cloud, alpha, depth_multiplier, classes, include_top, pooling, dropout
    )
...
def define_cnn_architecture_edge(iot_output, alpha, depth_multiplier, strides=(2, 2)):
    if PARTITION_SETING == 1:
        edge = _depthwise_conv_block(
            iot_output, 64, alpha, depth_multiplier, block_id=1
        )
        edge = _depthwise_conv_block(
            edge, 128, alpha, depth_multiplier, strides=strides, block_id=2
        )
        edge_output = _depthwise_conv_block(
            edge, 128, alpha, depth_multiplier, block_id=3
        )
    else:  # PARTITION_SETING == 2
        edge = _depthwise_conv_block(
            iot_output, 128, alpha, depth_multiplier, strides=strides, block_id=2
        )
        edge_output = _depthwise_conv_block(
            edge, 128, alpha, depth_multiplier, block_id=3
        )
...
def define_cnn_architecture_fog(edge_output, alpha, depth_multiplier):
    if PARTITION_SETING == 1:
        fog = _depthwise_conv_block(
            edge_output, 256, alpha, depth_multiplier, strides=(2, 2), block_id=4
        )
        fog = _depthwise_conv_block(fog, 256, alpha, depth_multiplier, block_id=5)
        fog = _depthwise_conv_block(
            fog, 512, alpha, depth_multiplier, strides=(2, 2), block_id=6
        )
        fog = _depthwise_conv_block(fog, 512, alpha, depth_multiplier, block_id=7)
        fog_output = _depthwise_conv_block(
            fog, 512, alpha, depth_multiplier, block_id=8
        )
    else:  # PARTITION_SETING == 2
        fog = _depthwise_conv_block(
            edge_output, 256, alpha, depth_multiplier, strides=(2, 2), block_id=4
        ...
        fog_output = _depthwise_conv_block(
            fog, 512, alpha, depth_multiplier, block_id=7
        )
    return fog_output


def define_cnn_architecture_cloud(
    fog_output, alpha, depth_multiplier, classes, include_top, pooling, dropout=1e-3
):
    if PARTITION_SETING == 1:
        cloud = _depthwise_conv_block(
            fog_output, 512, alpha, depth_multiplier, block_id=9
        )
        ...
        cloud = _depthwise_conv_block(
            cloud, 1024, alpha, depth_multiplier, strides=(2, 2), block_id=12
        )
        cloud = _depthwise_conv_block(cloud, 1024, alpha, depth_multiplier, block_id=13)
    else:  # PARTITION_SETING == 2
        cloud = _depthwise_conv_block(
            fog_output, 512, alpha, depth_multiplier, block_id=8
        )
    ...
    if include_top:
        ...
        cloud = layers.Conv2D(classes, (1, 1), padding="same", name="conv_preds")(cloud)
...
def imagenet_related_functions(
    weights, input_shape, include_top, classes, depth_multiplier, alpha
):
