...
def define_MLP_architecture_edge(img_input, hidden_units):
    if PARTITION_SETING == 1:
        edge_output = Dense(
            units=hidden_units, name="edge_output_layer", activation="relu"
        )(img_input)
    else:  # PARTITION_SETING == 2
        edge = Dense(units=hidden_units, name="edge_input_layer", activation="relu")(
            img_input
        )
        edge_output = Dense(
            units=hidden_units, name="edge_output_layer", activation="relu"
...
def define_MLP_architecture_fog2(fog2_input, hidden_units):
    if PARTITION_SETING == 1:
        fog2 = Dense(units=hidden_units, name="fog2_input_layer", activation="relu")(
            fog2_input
        )
        fog2_output = Dense(
            units=hidden_units, name="fog2_output_layer", activation="relu"
        )(fog2)
    else:  # PARTITION_SETING == 2
        fog2 = Dense(units=hidden_units, name="fog2_input_layer", activation="relu")(
            fog2_input
...
def define_MLP_architecture_fog1(fog1_input, hidden_units):
    if PARTITION_SETING == 1:
        fog1 = Dense(units=hidden_units, name="fog1_input_layer", activation="relu")(
            fog1_input
        )
        fog1 = Dense(units=hidden_units, name="fog1_layer_1", activation="relu")(fog1)
        fog1_output = Dense(
            units=hidden_units, name="fog1_output_layer", activation="relu"
        )(fog1)
    else:  # PARTITION_SETING == 2
        fog1 = Dense(units=hidden_units, name="fog1_input_layer", activation="relu")(
            fog1_input
...
def define_MLP_architecture_cloud(cloud_input, hidden_units, num_classes):
    if PARTITION_SETING == 1:
        ...
        cloud = Dense(units=hidden_units, name="cloud_layer_1", activation="relu")(
            cloud
        )
        cloud = Dense(units=hidden_units, name="cloud_layer_2", activation="relu")(
            cloud
        )
        cloud = Dense(units=hidden_units, name="cloud_layer_3", activation="relu")(
            cloud
        )
    else:  # PARTITION_SETING == 2
        cloud = Dense(units=hidden_units, name="cloud_input_layer", activation="relu")(
