...
def random_guess(train_labels, test_data):
    ...
    class_frequency_sorted_by_keys = list(
        dict(sorted(class_frequency.items())).values()
    )
    total_frequency = len(train_labels)
    # find relative frequency = class_frequency / total_frequency
    relative_class_frequency = [
        freq / total_frequency for freq in class_frequency_sorted_by_keys
    ]
...
def toss_coin(cumulative_frequency):
    ...
    for index in range(1, len(cumulative_frequency)):
        if (
            rand_num <= cumulative_frequency[index]
            and rand_num >= cumulative_frequency[index - 1]
        ):
            return index - 1
    return 0
    ...
    for i in range(1, num_nodes + 1):
        ...
        if horizontal_DNN:
            ...
            if name == "node1_input" or name == "node2_input" or name == "node3_input":
                if (
                    input_of_node_is_zero is True
                ):  # if input of any of the nodes 1,2,3 is zero, we have no_information_flow
                    return True
                ...
                if (
                    name == "node5_input"
                ):  # since we are processing the nodes in order, when we get here, it means we for nodes 1,2,3 (input_of_node_is_zero is False) and for node 4 (input_of_node_is_zero is True)
                    if input_of_node_is_zero is True:
                        return True
